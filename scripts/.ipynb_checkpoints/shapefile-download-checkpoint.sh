...
while IFS="," read read guid notam di ed ex tz city type pur timestamp
do
    temp_guid="${guid%\"}"
    temp_notam="${notam%\"}"
    temp_guid="${temp_guid#\"}"
    ...
    if [ -f "/tmp/$temp_notam.shp.zip" ]; then
        ...
        paste -d, <(echo "$guid") <(echo "$notam") <(echo $runtime) >> ${P}/newTFRids-archive.csv
    fi
...
if [ -f "${P}/newTFRids-archive.csv" ]; then
    while IFS="," read guid notam di ed ex tz city type pur timestamp
    do
        temp_guid="${guid%\"}"
        temp_notam="${notam%\"}"
        temp_guid="${temp_guid#\"}"
